    ...
    <div className="space-y-8 animate-fade-in">
          ...
          {data.dailyTrends.map((day, i) => {
            ...
            return (
              <div key={i} className="flex-1 group relative flex flex-col justify-end h-full">
                <div
                  className="w-full bg-zinc-600 rounded-sm transition-all group-hover:bg-zinc-500"
                  style={{ height: `${heightPx}px` }}
                />
                <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-zinc-800 px-3 py-2 rounded text-xs text-white opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">
                  <div className="font-mono">{day.jobs.toLocaleString()} jobs</div>
                  <div className="text-zinc-400">{day.senders} senders</div>
                </div>
              </div>
            );
          })}
        ...
        <div className="flex gap-3 mt-3">
          {data.dailyTrends.map((day, i) => (
            <div key={i} className="flex-1 text-center">
              ...
              <div className="text-zinc-300 font-mono text-sm mt-1">{day.jobs.toLocaleString()}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-zinc-900/50 border border-zinc-800/50 rounded-lg p-6">
        ...
        <div className="mt-6 pt-4 border-t border-zinc-800/50 flex items-center justify-between">
          <span className="text-zinc-500 text-sm">Average Confidence Score</span>
            ...
            {data.topOrigins.slice(0, 8).map((o, i) => (
              <div key={o.province} className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-zinc-300 text-sm">{o.province}</span>
                </div>
          ...
          <div className="space-y-3">
            {data.topDestinations.slice(0, 8).map((d, i) => (
              <div key={d.province} className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <span className="text-zinc-600 font-mono text-xs w-4">{i + 1}</span>
                  <span className="text-zinc-300 text-sm">{d.province}</span>
                </div>
